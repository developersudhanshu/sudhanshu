    ...
    createUser : (req, res) => {
        if(req, res){
            user.findOne({email:req.body.email}, function(err, usersData){
                if(usersData) {
                    res.status(201).send({success:false, message:"Email already Exists !", data:usersData});
                    console.log("Email already Exists !");
                }
                else{

        let panImage = "http://" + req.get("host") + "/upload/" + req.files[0].filename;       
        let aadhaarImage = "http://" + req.get("host") + "/upload/" + req.files[1].filename;
        let empPhoto = "http://" + req.get("host") + "/upload/" + req.files[2].filename;
        ...
        try{
            ...
            var encrypted = cipher.update(randomPassword, 'utf8', 'hex') + cipher.final('hex');
...
// ................
            ...
            createUser.save().then((result) => {
                ...
                jwt.sign({result},jwtKey,{expiresIn:'300s'},(error, token) => {
                    res.status(200).send({success:true ,message:"Create successfully !", data: token });
            ...
            text: 'Hii ' + req.body.name + '\nYour Email is = '+ req.body.email + ' & Password = ' + randomPassword
            };
    ...
    updateUser : async (req, res) => {
        ...
        let panImage ='';
        let empPhoto ='';
        if(req.body.is_aadhaar == "present" && req.body.is_pan == "present" && req.body.is_photo == "present"){
           
            aadhaarImage = "http://" + req.get("host") + "/upload/" + req.files[1].filename;
            data.aadhaar = aadhaarImage;
            panImage = "http://" + req.get("host") + "/upload/" + req.files[0].filename;
            data.pan = panImage;
            empPhoto = "http://" + req.get("host") + "/upload/" + req.files[2].filename;
            data.photo = empPhoto;
        }
        if(req.body.is_aadhaar == "present" && req.body.is_pan == "present" && req.body.is_photo == "not_present"){
        ...
        if(req.body.is_aadhaar == "present" && req.body.is_pan == "not_present" && req.body.is_photo == "present"){
           
            aadhaarImage = "http://" + req.get("host") + "/upload/" + req.files[1].filename;
            data.aadhaar = aadhaarImage;
            empPhoto = "http://" + req.get("host") + "/upload/" + req.files[0].filename;
            data.photo = empPhoto;
        }
        if(req.body.is_aadhaar == "not_present" && req.body.is_pan == "present" && req.body.is_photo == "present"){
           
            panImage = "http://" + req.get("host") + "/upload/" + req.files[0].filename;
            data.pan = panImage;
            empPhoto = "http://" + req.get("host") + "/upload/" + req.files[1].filename;
            data.photo = empPhoto;
        }
        if(req.body.is_aadhaar == "present" && req.body.is_pan == "not_present" && req.body.is_photo == "not_present"){
           
            aadhaarImage = "http://" + req.get("host") + "/upload/" + req.files[0].filename;
            data.aadhaar = aadhaarImage;
        }
        if(req.body.is_aadhaar == "not_present" && req.body.is_pan == "not_present" && req.body.is_photo == "present"){
           
            empPhoto = "http://" + req.get("host") + "/upload/" + req.files[0].filename;
            data.photo = empPhoto;
        }
        if(req.body.is_aadhaar == "not_present" && req.body.is_pan == "present" && req.body.is_photo == "not_present"){
    ...
    oneUser : async (req, res) => {
        ...
        employee_Details.aggregate([
            ...
            if(result){
                console.log("my result :",result)
                res.status(201).send({success:true, message:"Successfully !", users_data:result});
            }
        })
    },
                            ...
                            jwt.sign({data},jwtKey,{expiresIn:'300s'},(error, token) => {
                                ...
                                employee_Details.aggregate([           
                                    ...
                                    if(result){
                                        ...
                                        res.status(201).send({success:true, message:"Login Successfully !", data:token, users:data, users_data:result});
                                    }
                                })           
    ...
    allUserLeave : async (req, res) => {
        ...
        employee_Details.aggregate([
            ...
            if(result){
                // console.log("my result :",result)
                res.status(200).send({success:true ,message:"All User Leave data Successfully Show !", data: result});
                // res.send({data: result});
            }
        })
    }
